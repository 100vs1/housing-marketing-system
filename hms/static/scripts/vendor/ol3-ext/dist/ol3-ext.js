 ...
 *		- oninfo {function} callback on click on util button, if none no util button is shown
...
{	if (!this.tab_.length) return;
	...
	{	this.bar_.css("left", dx+"px").show();
		...
		$(".point-util .z", this.element).text(p[1]+"m");
		$(".point-util .dist", this.element).text((p[0]/1000).toFixed(1)+"km");
		$(".point-util .time", this.element).text(p[2]);
...
{	if (!options) options = {};
	...
	// Info
	$(".track-util .zmin", this.element).text(zmin.toFixed(2)+"m");
	$(".track-util .zmax", this.element).text(zmax.toFixed(2)+"m");
	if (d>1000)
	{	$(".track-util .dist", this.element).text((d/1000).toFixed(1)+"km");
	}
	else
	{	$(".track-util .dist", this.element).text((d).toFixed(1)+"m");
	}
	$(".track-util .time", this.element).text(ti);
...
{	if (resolution > this._maxResolution) return;
	...
	// SPARQL request: for more util @see http://fr.dbpedia.org/
...
{	if (resolution > this._maxResolution) return;
	var self = this;
	var bbox = ol.proj.transformExtent(extent, projection, "EPSG:4326");
	// Commons API: for more util @see https://www.mapillary.com/developer
...
{	if (resolution > this._maxResolution) return;
	var self = this;
	var bbox = ol.proj.transformExtent(extent, projection, "EPSG:4326");
	// Commons API: for more util @see https://commons.wikimedia.org/wiki/Commons:API/MediaWiki
...
* @param {} options
*	- camera {boolean} true to get camera util
*	- date {boolean} true to get photo date
*	- image {boolean} true to get image util
